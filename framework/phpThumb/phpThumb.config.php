...
//$PHPTHUMB_CONFIG['document_root'] = realpath((getenv('DOCUMENT_ROOT') && preg_match('#^'.preg_quote(realpath(getenv('DOCUMENT_ROOT'))).'#', realpath(__FILE__))) ? getenv('DOCUMENT_ROOT') : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace($phpThumb->osslash, '/', dirname(__FILE__))));
$PHPTHUMB_CONFIG['document_root'] = 'upload';
...
//$PHPTHUMB_CONFIG['cache_directory'] = $PHPTHUMB_CONFIG['document_root'].'/_cache/'; // set the cache directory to an absolute directory for all source images
...
$PHPTHUMB_CONFIG['cache_directory'] = '../../vars/cache/thumbs/';
...
$PHPTHUMB_CONFIG['cache_maxfiles'] = 2000; // delete least-recently-accessed cached thumbnails when more than [500] cached files are present (value is maximum number of cached files to keep)
...
$PHPTHUMB_CONFIG['cache_prefix'] = 'phpThumb_cache_'.@$_SERVER['SERVER_NAME'];
...
$PHPTHUMB_CONFIG['error_message_image_default'] = '/upload/error.png';       // Set this to the name of a generic error image (e.g. '/images/error.png') that you want displayed in place of any error message that may occur. This setting is overridden by the 'err' parameter, which does the same thing.
