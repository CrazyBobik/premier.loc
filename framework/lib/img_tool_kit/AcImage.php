...
class AcImage
{
	...
	private static $maxProportionLogo = 0.5;
 ...
 /**
	...
	/**
	 *
     * Задаёт изображению новые размеры.
	 *
	 * @return AcImage
	 * @throws IllegalArgumentException
	 *
	...
	public function simpleResize($newWidth, $newHeight) // Size $s || $width, $height  если 0 то сторона не указана 
	{
				
		//если не указана одна из сторон вычисляем её согласно пропорции 
							
		if( $newWidth == 0 ){
		
			$newWidth = (int)round(($newHeight/$this->getHeight())*$this->getWidth());
		
		}elseif( $newHeight == 0 ){
		
			$newHeight = (int)round(($newWidth/$this->getWidth())*$this->getHeight());
		...
		imagecopyresampled($imageNew, $this->getResource(), 0, 0, 0, 0, $newWidth, $newHeight, $this->getWidth(), $this->getHeight());
		
		$this->setResource($imageNew);
		$this->setSize(new Size($newWidth, $newHeight));
		
		return $this;
	
	}
	
	public function simpleResizeNewSize($newWidth, $newHeight) // Size $s || $width, $height  если 0 то сторона не указана, отдаёт новую сторону
	{
		...
		if( $newWidth == 0 ){
			...
			$r = (int)round(($newHeight/$this->getHeight())*$this->getWidth());
		
		}elseif( $newHeight == 0 ){
		
			$r = (int)round(($newWidth/$this->getWidth())*$this->getHeight());
	...
	/**
	 * "Ещё более умное" создание миниатюр вписыванием в заданные рамки согласно пропорциям.
	 *
	 * @param int ширина
	 * @param int высота
	 * @param float коэффициент превышения.
	...
	public function smartThumb($width, $height){ // $width, $height, [$c]
	
		$newImg = ImageCreateTrueColor ($width, $height); 
		
		//echo '-------------'.$height;
		
		imagefill($newImg , 0, 0, self::getBackgroundColor()->getCode());
		
		$imgWidth = $this->getWidth();
		$imgHeight = $this->getHeight();
		
		if($imgWidth >= $imgHeight){ // горизонтальная, приводим к размеру по ширене
						
			$bwidth =  $width;
			
			$bheight = (int) round(($width/$imgWidth)*$imgHeight);
			
			$y = floor(($height-$bheight)/2); //высота

			$this->simpleResize($bwidth, $bheight);			
			imagecopyresampled($newImg , $this->getResource(), 0, $y, 0, 0, $bwidth, $bheight, $bwidth, $bheight);
			
			/*
			echo '$bwidth'.$bwidth.'/n';
			echo '$bheight'.$bheight.'/n';
			echo '$width'.$width.'/n';
			echo '$height'.$height.'/n';
			*/
		
		}else{ // вертикальная, приводим к размеру по высоте
									
			$bheight =  $height;
						
			//$this->resizeByHeight($height);
			
			$bwidth = (int) round(($height/$imgWidth)*$imgHeight);		
						
			$x = floor(($width-$bwidth)/2); //высота
			
			$this->simpleResize($bwidth, $bheight);	
			
			imagecopyresampled($newImg , $this->getResource(), $x, 0, 0, 0, $bwidth, $bheight, $bwidth, $bheight);
							
		}	
		
		$this->setResource($newImg);
	...
	function sizeCrop($size=100)
	{	
	
		$size = (int)round($size);
		
		//Фіксована ширина та висота
		if ($this->getWidth() >= $this->getHeight()) { 		
		    $biggestSide = $this->getHeight(); 
		} else {	    	
		    $biggestSide = $this->getWidth();
		}
		$cropPercent = 1; // Збільшує зображення (лупа)  0.1 - 1 (1- пропорційно)
		$newWidth   = round($biggestSide*$cropPercent); 
		$newHeight  = round($biggestSide*$cropPercent);
		
		$x = round(($this->getWidth()-$newWidth )/2);
		$y = round(($this->getHeight()-$newHeight )/2);
		
		if($size > $this->getWidth() OR $size > $this->getHeight())
		{
		   if($this->getWidth()  > $this->getHeight()){ 	 
			   $size = $this->getHeight();			
			 }else{  
		...
		$newImageResource = imagecreatetruecolor($size, $size);
		
		imageAlphaBlending($newImageResource, false);
		imageSaveAlpha($newImageResource, true);
		
		imagecopyresampled($newImageResource, $this->getResource(), 0, 0, $x, $y, $size, $size,  $newWidth , $newHeight);
			
		$this->setResource($newImageResource);
		$this->setSize(new Size((int)$size,(int)$size));	
	 ...
	 * 0 1<br />
	 * 3 2	 
	 * @param $procents - занимать определённый процент от плащади картинки 
	 *
	...
	public function drawLogo($logo, $corner = null, $procents) // string $logo [$corner] || AcImage $logo [$corner]
	{
		if (is_null($corner))
		...
		if($procents){
			...
			$logoNewArea = ($this->getWidth()*$this->getHeight())*$procents;
			
			$logoSize = $logo->getSize();
		
			$logoArea = $logoSize->getHeight()*$logoSize->getWidth();
			
			$mult = sqrt($logoNewArea/$logoArea);
				
			$maxWidthLogo = (int)($logo->getWidth() * $mult);
		    $maxHeightLogo = (int)($logo->getHeight() * $mult);
		...
		//var_dump(( ($this->getWidth()* $this->getHeight())/( $logoSize->getWidth()* $logoSize->getHeight())));
		
		//exit();	
		
		$logo->simpleResize($maxWidthLogo, $maxHeightLogo);
	...
	/**
	 * Наносит лого на изображение в центре.
	 *
	 * @param mixed
	 * $logo2Proportion - Подтягивет логотип к необходимому размеру.
 	 *
	 * @see AcImage::TOP_LEFT
	 * @see AcImage::TOP_RIGHT
	 * @see AcImage::BOTTOM_LEFT
	...
	public function drawLogoCenter($logo, $logo2Proportion = false) // string $logo [$corner] || AcImage $logo [$corner]
	{
		...
		if( $logo2Proportion ){	
			...
			// находим меньшую сторону картинки на которую накладываем логотип
			if($this->getWidth()<=$this->getHeight()){ // высокая, берём от ширины 
				
			//echo "**********";	
			$newLogoWidth = (int)($this->getWidth() * self::$maxProportionLogo); 
				
				$h = $logo->simpleResizeNewSize( $newLogoWidth, 0);
			
				if($h>($this->getHeight()*0.5)){
				
					$newLogoHeight = (int)($this->getHeight() * self::$maxProportionLogo); 
					$logo->simpleResize( 0 , $newLogoHeight);
					
				}else{
				
					$logo->simpleResize($newLogoWidth, 0);
					
				}
						
			
			}else{ // широкая, берём от высоты 
				
			//echo "**********";	
			$newLogoHeight = (int)($this->getHeight() * self::$maxProportionLogo); 
				
				$w = $logo->simpleResizeNewSize( 0 , $newLogoHeight);
				
				if($w>($this->getWidth()*0.5)){
		...
		*/
		...
		// устанавливаем лого по центру 
		$x = round(($this->getWidth() - $logoSize->getWidth()) / 2);
		$y = round(($this->getHeight() - $logoSize->getHeight()) / 2);		
		
		//echo "-------". $logoSize->getWidth().'-----'.$logoSize->getHeight();
				
		imagealphablending($this->getResource(), true);
		
		imagecopy($this->getResource(), $logo->getResource(), $x, $y, 0, 0,
						$logoSize->getWidth(), $logoSize->getHeight());
