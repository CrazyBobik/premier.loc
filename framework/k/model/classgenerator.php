...
define('__K_CLASSGENERATOR_HEADER', '<?php // Generated by K_ClassGenerator'."\n");

class K_Model_ClassGenerator {
     ...
     * return array
     */
    public static function arrayPathSearch( &$array, $path ) {
    ...
    public function buildVariableValue( &$data = [] ) {
        if ( is_array($data) ) {
            if ( count($data) == 0 ) {
                return '[]';
            } else {
                ...
                foreach( $data as $key => &$dataElement ) {
                    $results .=  $this->buildVariableValue($key).'=>'.$this->buildVariableValue( $dataElement );
    ...
    public function buildVariable( $varName, &$varInfo ) {
        ...
        if ( isset($varInfo['value']) ) {
            $value = $this->buildVariableValue( $varInfo['value'] );// (string)$varInfo['value'];
        }
