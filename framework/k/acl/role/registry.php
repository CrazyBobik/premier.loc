...
class K_Acl_Role_Registry
{
     ...
     * @throws K_Acl_Role_Registry_Exception
     * @return K_Acl_Role_Registry Provides a fluent interface
     */
    public function add(K_Acl_Role_Interface $role, $parents = null)
    {
        $roleId = $role->getRoleId();

        if ($this->has($roleId)) {
           
            throw new K_Acl_Role_Registry_Exception("Role id '$roleId' already exists in the registry");
        }
        ...
        if (null !== $parents) {
            ...
            foreach ($parents as $parent) {
                try {
                    if ($parent instanceof K_Acl_Role_Interface) {
                        $roleParentId = $parent->getRoleId();
                    } else {
                        $roleParentId = $parent;
                    }
                    $roleParent = $this->get($roleParentId);
                } catch (K_Acl_Role_Registry_Exception $e) {
                    throw new K_Acl_Role_Registry_Exception("Parent Role id '$roleParentId' does not exist", 0, $e);
                }
    ...
    public function get($role)
    {
        ...
        if (!$this->has($role)) {
            ...
            throw new K_Acl_Role_Registry_Exception("Role '$roleId' not found");
        }
    ...
    public function inherits($role, $inherit, $onlyParents = false)
    {
        ...
        try {
            ...
            throw new K_Acl_Role_Registry_Exception($e->getMessage(), $e->getCode(), $e);
        }
