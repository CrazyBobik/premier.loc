...
class Formbuilder{
	...
	public function get_encoded_form_array(){
		return array('form_structure'=>json_encode($this->_form_array['form_structure']));
	}
	 ...
	 * @access public
	 */
	public function render_json($send_headers = true){
		if ($send_headers)
		{
			header("Content-Type: application/json");
		}
		
		return json_encode( $this->_form_array );
	}
	...
	protected function loadInputText($field){
		...
		$html .= sprintf('<label for="%s">%s</label>' . "\n", $this->elemId($field['values']['lable']), $field['values']['lable']);
		$html .= sprintf('<input type="text" id="%s" name="%s"  value="%s" placeholder="%s" />' . "\n",
								$this->elemId($field['values']['lable']),
								$this->elemId($field['values']['lable']),
								$this->getPostValue($this->elemId($field['values']['lable'])),
                                $this->elemId($field['values']['placeholder']));
		$html .= '</li>' . "\n";
	 ...
	 * @return string
	 */
	protected function loadFile($field){
		...
		$html .= sprintf('<li class="%s%s" id="fld-%s">' . "\n", $this->elemId($field['cssClass']), $field['required'], $this->elemId($field['values']));
		$html .= sprintf('<label for="%s">%s</label>' . "\n", $this->elemId($field['values']['lable']), $field['values']['lable']);
		$html .= sprintf('<input type="file" id="%s" name="%s" />' . "\n",
	...
	protected function loadTextarea($field){
		...
		$html .= sprintf('<li class="%s%s" id="fld-%s">' . "\n", $this->elemId($field['cssClass']), $field['required'], $this->elemId($field['values']));
		$html .= sprintf('<label for="%s">%s</label>' . "\n", $this->elemId($field['values']['lable']), $field['values']['lable']);
		$html .= sprintf('<textarea id="%s" name="%s" rows="5" cols="50"  placeholder="%s" ></textarea>' . "\n",
								$this->elemId($field['values']['lable']),
								$this->elemId($field['values']['lable']),
							    $this->elemId($field['values']['placeholder']));
		$html .= '</li>' . "\n";
