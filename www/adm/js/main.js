...
function delAdmin(id){
    ...
    if (confirm("Вы действительно хотите удалить этого сотрудники?")) {
    
        $.ajax({
    		type: 'post',
    		url: "/admin/acladmin/del",
            data: {id:adminId},
    		success: function() {
...
$(function(){
     ...
     $('form.ajax-form').live('submit', function()
		...
		$('#admins-table-wrapper').ajaxLeaf({
			loadUrl: '/admin/acladmin/load/',
			itemsContainer: '#items',
			"renderItems": function(items) {
				var itemsHtml = [];
				$.each(items, function(id, item) {
	
					itemsHtml.push('<tr id="admin-row-' + id + '" class="admin-row"> <td>' + item.name + '</td><td>' + item.login + '</td><td>' + item.email + '</td><td>' + item.roles + '</td><td><img class="table-btn" src="/adm/img/userRemove.png" onclick="delAdmin(\'' + id + '\')" title="Удалить" alt="Удалить" /><img src="/adm/img/formbuilder/edit.png" class="table-btn" onclick="editAdmin(\'' + id + '\')" title="Редактировать" alt="Редактировать" /></td></tr>');
 ...
 */
	...
	function addAdmin() {
		...
		$('#add-admin-title').html('Добавление нового сотрудника');
	...
	/*function delUser(userID) {
		if (confirm('Вы подтверждаете удаление?')) {
			$.post('/admin/acluser/del', 'delusrid=' + userID, function(data) {
				if (!data.error) {
	...
	function adminInfoSubmit() {
		...
		$.post('/admin/acladmin/save', str, function(data) {
			if (data.error == true) {
				showValidateErrors(data.msg);
			} else {
				clearAllRoles();
				...
				for (var key in data.admin.roles) {
					...
					adminRoles[key] = $('#add-roles option[value="' + val + '"]').text();
				}
...
/* var tdArray=[];
             ...
             tdArray.push('<img class="table-btn" src="/adm/img/userRemove.png" onclick="delUser(\'' + data.user.id + '\')" title="Удалить" alt="Удалить" /><img src="/adm/img/formbuilder/edit.png" class="table-btn" onclick="editUser(\'' + data.user.id + '\')" title="Редактировать" alt="Редактировать" />');
              ...
              $(newRow).attr('id','user-row-' + data.user.id);*/
					...
					$('#acl-admins tbody').prepend(adduser);
	...
	function editAdmin(adminId) {
		...
		$('#add-admin-title').html('Редактирование сотрудника:<b> ' + adminName + '</b>');
		$('#add-admin').animate({
			opacity: 'show',
			height: 'show'
		}, 'slow');
	...
	function delRole(roleID) {
		if (confirm('Вы подтверждаете удаление?')) {
			$.post('/admin/aclrole/del', 'delroleid=' + roleID, function(data) {
				if (!data.error) {
	...
	function roleInfoSubmit() {
		...
		$.post('/admin/aclrole/save', str, function(data) {
			if (data.error == true) {
				showValidateErrors(data.msg);
			} else {
...
// dev секция временно будет находиться тут 
...
$(function(){
    ...
    $('#crud-select').on('dblclick', 'option', function(){
        
           $.post('/dev/addcrud/edit',{name:$(this).val()}, function(data){
            
                  $('#crud-form-cont').html(data);
                  
                },'html');    
        
    });
    
    $('#crud-form-cont').on('click', '#crud_generate', function(){
        
            var crud = $('#crud-select option:selected').val();
           
            $.post('/dev/addcrud/generate', {name:crud}, function(data){
                    
                        var nNote = '#flash-msg-nNote';    
...
$(function(){
    $('body').on('keyup','#node_name',function(){
	    var result= translitLower($('#node_name').val().toLowerCase());
		$('#tree_name').val(result); 
        return false;
    });
});
*/