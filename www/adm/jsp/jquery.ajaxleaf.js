		...
		$.fn.ajaxLeaf = function(opt) {
			var opt = $.extend({
				...
				"renderItems": function(items) {
					var itemsHtml = [];
					$.each(items, function(id, admin) {
						itemsHtml.push('<tr id="admin-row-' + id + '" class="admin-row"> <td>' + admin.name + '</td><td>' + admin.login + '</td><td>' + admin.email + '</td><td>' + admin.roles + '</td><td><img class="table-btn" src="/adm/img/userRemove.png" onclick="delAdmin(\'' + id + '\')" title="Удалить" alt="Удалить" /><img src="/adm/img/formbuilder/edit.png" class="table-btn" onclick="editAdmin(\'' + id + '\')" title="Редактировать" alt="Редактировать" /></td></tr>');
					});
					return itemsHtml;
				},
  			...
  			return $(this).each(function() {
				...
				var loadRows = function(filter, page, onPage){
					  	 ...
					  	 $.ajax({		
									    type: "POST",
									    url: opt.loadUrl,
									    // data: 'filter=' + filter + '&page=' + page + '&onPage=' + onPage + '&' + $(opt.xInfoHolder).text()+'&'+opt.collectInfo(wrapper),
									    dataType: "json"
										...
										$nav_panels = wrapper.find('#acl-admins_paginate');
